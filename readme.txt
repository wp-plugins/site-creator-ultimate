...
Enhance the look of your blog or website or create a great looking front page.
Make your website or blog look professional. 

== Description ==

This is an all around plugin that allow you lots of control how your front page, posts, pages and website looks. For example it can enhance the look of your blog or front page, or make a website look like a professional magazine or news website. 

= Usage =

By pasting a shortcode on a page you can display a layout which is a number of posts that can be themed. Each layout can be edited with a number of options such as how many posts to show, image & text options, adding effects and much more. This allows for the creation of a front page, sub pages, blog, category pages or widgets. A front page can be made looking like a magazine or a news website. Several effects can be added such as pop up, text over image and image swap effects. 
...
* Build a front page, Sub pages, Create widgets to make you website look great
...
* CSS style backed allows you to change all aspects
...
This plugin works by using layouts. A layout is basicly a list of posts that can have a specific look. The layout (or the posts) can then be displayed on a page, post or widget. To display it a short code is used. For example to display layout 1 the following short code can be used: 

[sitecreator show="1"] 

There are up to 20 layouts that can be used. The layout in its default state will show 3 posts with a simple layout with no filters or effects which is a starting point. To this more elements, filters and effects can be added. 
...
1. Click on "add a new page" which will be your new front page
2. Add one or several Short codes in the page to display each layout and to display the posts  

for example to use only layout 1 we can use

[sitecreator show="1"]

for example if we want o use layout 1, 2 and 3 we can use

[sitecreator show="1"]
[sitecreator show="2"]
[sitecreator show="3"]

3. Under Settings -> Reading Settings, mark the static page and in the drop down list the page you just created
4. Edit the layout 1 to 3


= Example: How do I create a new front page for my blog? =

1. Click on "add a new page" which will be your new front page
2. Add one Short code in the page to display a layout and to display your posts  
...
3. Under Settings -> Reading Settings, mark the static page and in the drop down list the page you just created
4. Edit the layout to your liking
...
The widget can be customized with visual nice image effects. In this example we will create a widget to show the latest posts from certain category

1. Choose layout 1 or a layout you have not been using yet
2. Change the layout to your pleasing by adding title, excerpt and changing image size etc. Important: Make sure that the image and the post is fits into your sidebar!
3. Under "Show posts only from category" select the category you want show posts from
4. save

Now we have created a layout, but to create a widget with this layout we need to do the following

5. Under Appearance -> widget  drag a text widget (that allows you to add arbitrary text or HTML) to your desired sidebar.
...
Layout space bottom - add a space or hr seperator tag under the layout to separate it from other layouts. Useful if you add several layouts on a page.
...
Width & Height - set the width and height of the pop up. NOTE: if height is 0 then the height will be set automaticly. 
